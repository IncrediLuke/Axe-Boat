...
// Whole MODDING API [v0.9.4] - It makes this build work!!! (Dont ask why shit is unminified on purpose ) - Rizsim Studios
let ShakeEffectOn = null === localStorage.getItem("ShakeEffectOn") || "true" === localStorage.getItem("ShakeEffectOn"),
    LyricsOn = null === localStorage.getItem("LyricsOn") || "true" === localStorage.getItem("LyricsOn"),
    BGorFadeOutsOn = null === localStorage.getItem("BGorFadeOutsOn") || "true" === localStorage.getItem("BGorFadeOutsOn"),
    ModSoundsOn = null === localStorage.getItem("ModSoundsOn") || "true" === localStorage.getItem("ModSoundsOn"),
    ConfettiEffectOn = null === localStorage.getItem("ConfettiEffectOn") || "true" === localStorage.getItem("ConfettiEffectOn"),
    CustomCursorsOn = null === localStorage.getItem("CustomCursorsOn") || "true" === localStorage.getItem("CustomCursorsOn");
null === localStorage.getItem("realEnding") && localStorage.setItem("realEnding", "false");
let realEnding = "true" === localStorage.getItem("realEnding"),
    flipped = !1,
    achiNotificationDot = parseInt(localStorage.getItem("achiNotificationDot")) || 0,
    totalBonusesWatched = parseInt(localStorage.getItem("totalBonusesWatched")) || 0,
    totalVersionsOppened = parseInt(localStorage.getItem("totalVersionsOppened")) || 0,
    ...
    if (ShakeEffectOn) {
        ...
        const l = (s) => {
            ...
            const c = s - i,
                ...
                r = (2 * Math.random() - 1) * o * f;
            (e.style.transform = `translate(${u}px, ${r}px)`), c < n ? requestAnimationFrame(l) : (e.style.cssText = t);
        };
...
function adjustScale() {
    ...
    if (o && n && !o.classList.contains("active") && !e) {
        ...
        o.style.transform = n <= 400 ? "scale(0.7)" : n <= 500 ? "scale(0.8)" : "scale(1)";
    }
}
function PlaySFX(o) {
    ModSoundsOn &&
        new Audio(`./${app.folder}sound/mod/${o}`).play().catch((n) => {
            boxDialog.open(`Oops! We can't play the sound file "${o}".<br><br>Make sure the file exists at:<br><b>${app.folder}sound/mod/${o}</b>`, "~(>_<。)＼");
        });
}
window.addEventListener("load", adjustScale), window.addEventListener("resize", adjustScale);
...
function checkForGriser(o) {
    if (o && o.classList.contains("griser")) {
        ...
        showNotification(e, "polo"), console.log(e), totalPolosUsed++, localStorage.setItem("totalPolosUsed", totalPolosUsed);
    }
}
...
function handleBonusClassChanges(o) {
    ...
    if (n && e) {
        ...
        showNotification(`DEBUG: v${getVersionFromURL()} Bonus ${n} is playing!`, "bonus");
    }
    if (n && e) {
        const n = new MutationObserver(() => {
            if (!o.classList.contains("playing")) {
                const e = o.id.split("-")[2];
                showNotification(`DEBUG: v${getVersionFromURL()} Bonus ${e} has finished playing!`, "bonus-finished"), n.disconnect(), totalBonusesWatched++, localStorage.setItem("totalBonusesWatched", totalBonusesWatched);
            }
        });
...
const bonusObserver = new MutationObserver((o) => {
        o.forEach((o) => {
            "attributes" === o.type && "class" === o.attributeName && handleBonusClassChanges(o.target);
        });
    }),
...
function checkForPictoClassChange(o) {
    if (o) {
        ...
        if (o.classList.contains("picto") && !n) {
            const n = o.getAttribute("data-picto-num");
            showNotification(`DEBUG: v${getVersionFromURL()} Polo ${n} is not unused!`, "polo");
        }
    }
}
...
const pictoClassObserver = new MutationObserver((o) => {
    o.forEach((o) => {
        "attributes" === o.type && "class" === o.attributeName && checkForPictoClassChange(o.target);
    });
});
function startObservingPictoClassChanges() {
    document.querySelectorAll(".picto").forEach((o) => {
        pictoClassObserver.observe(o, { attributes: !0, attributeFilter: ["class"] });
    });
}
...
function CreateElement(o, n = "div", e = "") {
    if (document.getElementById(o)) return void console.warn(`Element with ID '${o}' already exists.`);
...
showNotification(`DEBUG: v${getVersionFromURL()} just started!`),
    ...
    fetch("versions.xml")
        .then((o) => o.text())
        .then((o) => {
            let n = new DOMParser().parseFromString(o, "text/xml").getElementsByTagName("line"),
            ...
            for (let s = 0; s < n.length; s++) {
                ...
                for (let u = 0; u < f.length; u++) {
                    let r = f[u].getAttribute("ID"),
                        a = f[u].getAttribute("color"),
                    ...
                    B.classList.add("vicon"), (B.id = "icon" + r), (B.dataset.name = d), (B.dataset.color = a), (B.dataset.locked = g);
                    let E = document.createElement("div");
                    P ? ((E = document.createElement("img")), (E.id = "customicon"), (E.src = P), (E.alt = d)) : E.classList.add("img"),
                        B.appendChild(E),
                        (B.innerHTML += V ? `<div id="customIconName" style="color: ${a};">${d}</div>` : '<div class="txt"></div> \x3c!-- Exists but empty --\x3e'),
                        "true" !== g &&
                    ...
                    function i(o) {
                        "true" === B.dataset.locked
                            ? (o.preventDefault(),
                              showNotification("Trying to open locked icon " + r),
                              showNotification("Trying to open locked icon", "locked"),
                              RemoveElement("lock-all"),
                              RemoveElement("fade-all"),
                              document.querySelectorAll('.vicon[data-locked="true"]').forEach((o) => {
                            ...
                            : (o.stopPropagation(),
                              ...
                              OnDelayIn(900, () => {
                                  setTimeout(() => {
                                      window.location.href = "app.html?v=" + r;
                                  }, 10);
                              }));
                    }
                    (p[`colV${r}`] = a), localStorage.setItem("iconColors", JSON.stringify(p)), B.addEventListener("click", i, { passive: !1 }), B.addEventListener("touchstart", i, { passive: !1 });
                }
            }
            let l = document.createElement("style");
            (l.innerHTML = `:root {\n${t}}`), document.head.appendChild(l), applyStoredColors();
        })
...
function QuickMessage(o, n = 2e3) {
    if (LyricsOn) {
        ...
        e.classList.add("quickmessage"),
            ...
            (e.style.zIndex = "9999"),
            (e.style.opacity = "0"),
            (e.style.transition = "opacity 0.3s ease, transform 0.3s ease, bottom 0.3s ease"),
            document.body.appendChild(e),
            messageQueue.unshift(e),
            updateMessagePositions(),
            setTimeout(() => {
                (e.style.opacity = "1"), (e.style.transform = "translateX(-50%) translateY(0px)");
            }, 10),
            setTimeout(() => {
                (e.style.opacity = "0"),
                    (e.style.transform = "translateX(-50%) translateY(20px)"),
...
function OnlyOnce(o, n) {
    let e = JSON.parse(localStorage.getItem(`executedItems_${RegisterMod}`)) || {};
    e[o] || (n(), (e[o] = !0), localStorage.setItem(`executedItems_${RegisterMod}`, JSON.stringify(e)));
}
function ClearItemID(o) {
    let n = JSON.parse(localStorage.getItem(`executedItems_${RegisterMod}`)) || {},
        e = JSON.parse(localStorage.getItem("storage")) || {};
    n[o] && (delete n[o], localStorage.setItem(`executedItems_${RegisterMod}`, JSON.stringify(n))), e[o] && (delete e[o], localStorage.setItem("storage", JSON.stringify(e)));
}
...
function ReplacePictoID(o, n) {
    ...
    e ? (e.setAttribute("data-picto-num", n), (e.id = `picto${n}`), console.log(`Picto ID and data-picto-num replaced: picto${o} -> picto${n}`)) : console.warn(`Element with ID 'picto${o}' not found.`);
}
function RemovePicto(o) {
    const n = document.getElementById(`picto${o}`);
    n ? (n.remove(), console.log(`Picto element with ID 'picto${o}' removed.`)) : console.warn(`Element with ID 'picto${o}' not found.`);
}
function AddPicto(o, n) {
    const e = document.getElementById("box-picto");
    if (e) {
        let t;
        if ((1 === n ? (t = e.querySelector(".pictoline.top")) : 2 === n && (t = e.querySelector(".pictoline.bot")), t)) {
            const e = document.createElement("div");
            e.classList.add("picto"), (e.id = `picto${o}`), e.setAttribute("data-picto-num", o);
            const i = document.createElement("div");
            i.classList.add("bck"), (i.style.backgroundImage = `url("./${app.folder}img/game-picto@2x.png")`);
            const l = document.createElement("div");
            l.classList.add("hitzone"), e.appendChild(i), e.appendChild(l), t.appendChild(e), console.log(`Picto element with ID 'picto${o}' added to line ${n}.`);
...
function unHide(o) {
    ...
    n ? ((n.style.display = ""), (n.dataset.hidden = "false"), console.log(`Icon with ID 'icon${o}' is now visible.`)) : console.warn(`Element with ID 'icon${o}' not found.`);
}
function Hide(o) {
    const n = document.getElementById(`icon${o}`);
    n ? ((n.style.display = "none"), (n.dataset.hidden = "true"), console.log(`Icon with ID 'icon${o}' is now hidden.`)) : console.warn(`Element with ID 'icon${o}' not found.`);
}
function ChangeMenuTitle(o) {
    const n = document.querySelector(".container .title"),
        e = document.getElementById("sp-choose");
    n ? ((n.textContent = o), console.log(`Menu title changed to: ${o}`)) : console.log(`Cannot find ".container .title" Element wont change to "${o}"`),
        e ? ((e.textContent = o), console.log(`Menu title changed to: ${o}`)) : console.log(`Cannot find ".container .title" Element wont change to "${o}"`);
}
function unLock(o) {
    const n = document.getElementById(`icon${o}`);
    n
        ? ((n.dataset.locked = "false"),
          n.querySelector(".bul") || (n.innerHTML += '\n                <div class="bul">\n                  <svg class="icn-svg"><use xlink:href="#ic-check"></use></svg>\n                </div>\n            '),
...
function Catch(o, n) {
    ...
    e[o] || (e[o] = { callbacks: [], active: !1 }), "function" == typeof n && (e[o].callbacks.push(n), e[o].active && n()), localStorage.setItem("storage", JSON.stringify(e));
}
function OnClick(o, n) {
    const e = document.getElementById(o);
    e && "function" == typeof n ? e.addEventListener("click", n) : console.error(`Element with ID '${o}' not found.`);
}
            ...
            (e.innerHTML = ""),
                n.forEach((o) => {
                    ...
                    (n = n.replace(/\(\!line\)/g, "<br>")),
                        ...
                        (n = n.replace(/\(\!color\/#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})\/(.*?)\)/g, '<span style="color:#$1;">$2</span>')),
                        (n = n.replace(/\(\!size\/(\d+px)\/(.*?)\)/g, '<span style="font-size:$1;">$2</span>')),
                        (n = n.replace(/\(\!emoji\/(.*?)\)/g, (o, n) => `<img src="mod-credits/emoji/${n}" class="credit-emoji" />`)),
                        (n = n.replace(
                        ...
                        (n = n.replace(/\(\!youtube\/(.*?)\)/g, (o, n) => {
                            let e = "";
                            const t = n.match(/(?:youtube\.com\/(?:.*v=|embed\/|v\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})/);
                            return t && (e = t[1]), e ? `<iframe class="credit-video" src="https://www.youtube.com/embed/${e}" frameborder="0" allowfullscreen></iframe>` : '<span style="color: red;">[Invalid YouTube Link]</span>';
                        }));
                    ...
                    (f.id = "text-credits"), (f.innerHTML = n), s.appendChild(f), e.appendChild(s);
                });
        })
        .catch((o) => {
...
function updateShakeEffectSwitch() {
    var o = document.querySelector("#pop-advsetting #param-disableshake .bt-onoff");
    o && o.classList.toggle("active", ShakeEffectOn);
}
function updateLyricsSwitch() {
    var o = document.querySelector("#pop-advsetting #param-disablelyrics .bt-onoff");
...
function updateModSoundsEffectSwitch() {
    const o = document.querySelector("#pop-advsetting #param-displaymodsounds .bt-onoff");
    o && o.classList.toggle("active", ModSoundsOn);
}
function updateParticlesSwitch() {
    const o = document.querySelector("#pop-advsetting #param-particleseffect .bt-onoff");
...
function resetEverything() {
    OnDelayIn(300, () => {
        localStorage.clear(),
            boxDialog.open("All data has been erased!<br>\n             The game will now reset to its first-time launch state.", "Info", !1),
            OnDelayIn(5500, () => {
                location.reload();
            });
    });
    ...
    OnDelayIn(300, () => {
        localStorage.getItem(o)
            ? (localStorage.removeItem(o),
              ...
              boxDialog.open(`All achievements in <b>${RegisterMod}</b> have been reset.<br>\n                 The game will restart in 2 seconds.`, "Info", !1),
              OnDelayIn(2500, () => {
                  location.reload();
              }))
            : boxDialog.open("No achievements found for this mod.", "", [STR("bt.gotit")]);
    });
}
function getElementPosition(o) {
    ...
    return { x: (e.left + e.width / 2) / window.innerWidth, y: (e.top + e.height / 2) / window.innerHeight };
}
function MakeConfetti(o = 2, n = 360, e = 1, t = 200, i = 100, l, s) {
    if (ConfettiEffectOn) {
        const c = getElementPosition(`#${l}`);
        confetti({ spread: n, ticks: t, gravity: e, decay: 0.94, startVelocity: 20, particleCount: i, origin: c, scalar: o, shapes: ["image"], shapeOptions: { image: [{ src: s, width: 32, height: 32 }] } });
    }
}
function setCustomCursor(o) {
    if (CustomCursorsOn) {
        let n = document.createElement("style");
        (n.innerHTML = "* { cursor: none !important; }"), document.head.appendChild(n);
...
let loopIntervals = [];
function onLoop(o, n) {
    if ("number" != typeof app.looptime || app.looptime <= 0) return void console.error("app.looptime must be a positive number.");
    if (o <= 0) return void boxDialog.open("Loop multiplier must be greater than zero.");
    const e = app.looptime * o,
        t = setInterval(n, e);
    loopIntervals.push(t);
}
function offLoop() {
    loopIntervals.forEach(clearInterval), (loopIntervals = []);
}
function Zoom(o, n, e) {
    ...
    const i = o / 100;
    n
        ? ((t.style.transition = "transform 1s cubic-bezier(0.55, 0.09, 0.68, 0.53)"), (t.style.transform = `scale(${i})`))
        : e
        ? ((t.style.transition = "transform 1s cubic-bezier(0.22, 0.61, 0.36, 1)"), (t.style.transform = `scale(${i})`))
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    new MutationObserver((e) => {
        e.forEach((e) => {
            "attributes" === e.type && "class" === e.attributeName && (o.classList.contains("animate") ? n.classList.add("animate") : n.classList.remove("animate"));
        });
    ...
    OnClick("ShakeeffectsButton", () => {
        (ShakeEffectOn = !ShakeEffectOn), updateShakeEffectSwitch(), localStorage.setItem("ShakeEffectOn", ShakeEffectOn);
    }),
    updateShakeEffectSwitch(),
    OnClick("LyricsDebulgButton", () => {
        (LyricsOn = !LyricsOn), updateLyricsSwitch(), localStorage.setItem("LyricsOn", LyricsOn);
    }),
    updateLyricsSwitch(),
    OnClick("bgchangesbutton", () => {
        (BGorFadeOutsOn = !BGorFadeOutsOn), updateBGchangesEffectSwitch(), localStorage.setItem("BGorFadeOutsOn", BGorFadeOutsOn);
    }),
    updateBGchangesEffectSwitch(),
    OnClick("displaymodsoundsButton", () => {
        (ModSoundsOn = !ModSoundsOn), updateModSoundsEffectSwitch(), localStorage.setItem("ModSoundsOn", ModSoundsOn);
    }),
    ...
    OnClick("particleseffectbutton", () => {
        (ConfettiEffectOn = !ConfettiEffectOn),
            ...
            SomeTimes(0.1, () => {
                MakeConfetti(2.5, 360, 0, 400, 50, "custom-cursor", "https://upload.wikimedia.org/wikipedia/commons/thumb/f/fb/718smiley.svg/1200px-718smiley.svg.png"),
                    OnDelayIn(1e3, () => {
                        MakeConfetti(2.5, 360, 0, 400, 41, "custom-cursor", "https://upload.wikimedia.org/wikipedia/commons/thumb/f/fb/718smiley.svg/1200px-718smiley.svg.png");
                    }),
                    OnDelayIn(2e3, () => {
                        MakeConfetti(2.5, 360, 0, 400, 29, "custom-cursor", "https://upload.wikimedia.org/wikipedia/commons/thumb/f/fb/718smiley.svg/1200px-718smiley.svg.png");
                    }),
                    OnDelayIn(3300, () => {
                        MakeConfetti(2.5, 360, 0, 400, 11, "custom-cursor", "https://upload.wikimedia.org/wikipedia/commons/thumb/f/fb/718smiley.svg/1200px-718smiley.svg.png");
                    });
            });
    }),
    updateParticlesSwitch(),
    OnClick("Cursorbuttoneffect", () => {
        (CustomCursorsOn = !CustomCursorsOn), updateCursorsEffectSwitch(), localStorage.setItem("CustomCursorsOn", CustomCursorsOn);
    }),
    updateCursorsEffectSwitch(),
    OnClick("info-bgchanges", () => {
        boxDialog.open("Disables all dynamic background effects.<br><b>Warning:</b> Do not enable this if you have epilepsy!", "", [STR("bt.gotit")]);
    }),
    OnClick("info-shakeEffects", () => {
        boxDialog.open("Disables all screen-shaking effects in the mod.<br>Useful if they are overused or cause discomfort.", "", [STR("bt.gotit")]);
    }),
    OnClick("info-disablelyrics", () => {
        boxDialog.open("Disables the lyrics text that may appear<br>at the bottom of the screen.", "", [STR("bt.gotit")]);
    }),
    OnClick("info-displaymodsounds", () => {
        boxDialog.open("Disables custom sounds added by the mod.<br>Vanilla Incredibox sounds will not be affected.", "", [STR("bt.gotit")]);
    }),
    OnClick("info-particleseffect", () => {
        boxDialog.open("Disables confetti and particle effects.<br>Recommended for low-performance devices.", "", [STR("bt.gotit")]);
    }),
    OnClick("info-Cursor", () => {
        boxDialog.open("Disables the custom cursor that may appear in this mod. (PC only)", "", [STR("bt.gotit")]);
    }),
    OnClick("info-achi-reset", () => {
        boxDialog.open("Resets all achievement progress in this mod.<br><b>This action cannot be undone.</b>", "", [STR("bt.gotit")]);
    }),
    ...
    OnClick("info-all-reset", () => {
        boxDialog.open("Resets everything: bonuses, achievements, all your mixes, and progress.<br>\n         <b>This applies to all mods and the official game.</b>", "", [STR("bt.gotit")]);
    }),
    ...
    onKeyHold("I", "$", function () {
        let o = app.animearray.map((o) => `\n        <div>\n            <b>${o.name}</b> / \n            <span style="color: #${o.color}">■</span> /\n            2 loops: ${o.uniqsnd ? "No" : "Yes"}\n        </div>\n    `).join("");
        QuickMessage(`\n        <span style="color: ${app.col1}">Character's INFO</span><br><hr><br>\n        <div class="modding-build">\n            ${o}\n        </div>\n    `, 1e4);
    }),
    onKeyHold("I", "%", function () {
        let o = (parseInt(localStorage.getItem("totalTimeSpent")) || 0) + Math.floor((Date.now() - sessionStartTime) / 1e3),
            n = Math.floor(o / 3600),
            e = Math.floor((o % 3600) / 60),
            t = o % 60,
            i = `${n.toString().padStart(2, "0")}:${e.toString().padStart(2, "0")}:${t.toString().padStart(2, "0")}`;
        QuickMessage(
...
function openDatabase(o) {
    ...
    (n.onupgradeneeded = function (o) {
        ...
        n.objectStoreNames.contains("modIcons") || n.createObjectStore("modIcons", { keyPath: "modName" }), n.objectStoreNames.contains("modAchievements") || n.createObjectStore("modAchievements", { keyPath: "modName" });
    }),
...
function registerMod() {
    ...
    console.log("Starting mod registration..."),
        fetch(RegisterModIcon)
            .then((o) => {
                if ((console.log("Fetched icon, status:", o.status), !o.ok)) throw new Error("Icon file not found!");
                return o.blob();
            })
            .then((n) => {
                if ((console.log("Icon Blob received, size:", n.size, "type:", n.type), !(n.size > 1536e3))) return (o = n), fetch(RegisterModBackground);
                OnDelayIn(500, function () {
                    boxDialog.open("Icon is too large! Please use an image under 1.5MB.", "Icon Upload Failed");
                });
            })
            .then((o) => {
                if ((console.log("Fetched background, status:", o.status), !o.ok)) throw new Error("Background file not found!");
                return o.blob();
            })
            .then((e) => {
                if ((console.log("Background Blob received, size:", e.size, "type:", e.type), !(e.size > 256e4))) return (n = e), processImages(o, n);
                OnDelayIn(500, function () {
            ...
            .then(({ base64Icon: o, base64Background: n }) => {
                ...
                (e.onerror = function () {
                    ...
                    (e.onsuccess = function (e) {
                        e.target.result.transaction("modIcons", "readwrite").objectStore("modIcons").put({ name: RegisterMod, version: RegisterModVersion, developer: ModDeveloper, icon: o, background: n, lastPlayed: Date.now() }),
                            console.log("Mod saved successfully!");
                    });
            })
            .catch((o) => {
                console.error("Fetch or processing error:", o), boxDialog.open("Failed to load mod assets: " + o.message, "Error");
            });
}
...
function updateLastPlayed(o) {
    ...
    (n.onsuccess = function (n) {
        let e = n.target.result.transaction("modIcons", "readwrite").objectStore("modIcons"),
            t = e.get(o);
        (t.onsuccess = function () {
            let n = t.result;
            n ? ((n.lastPlayed = Date.now()), e.put(n), console.log(`Updated last played time for ${o}`)) : console.warn(`Mod "${o}" not found in IndexedDB.`);
        }),
            (t.onerror = function () {
...
function getLastPlayed(o, n) {
    const e = indexedDB.open("ModLibraryDB", 2);
    (e.onsuccess = function (e) {
        let t = e.target.result.transaction("modIcons", "readonly").objectStore("modIcons").get(o);
        (t.onsuccess = function () {
            let o = t.result;
            if (o && o.lastPlayed) {
            ...
            (t.onerror = function () {
                console.error("Error retrieving mod data from IndexedDB."), n("Unknown");
            });
    }),
        (e.onerror = function () {
...
(dbRequest.onupgradeneeded = function (o) {
    let n = o.target.result;
    n.objectStoreNames.contains("modIcons") || n.createObjectStore("modIcons", { keyPath: "name" });
}),
    ...
    o &&
        (achiNotificationDot > 0
            ? (o.classList.add("show"),
              (o.innerText =
                  achiNotificationDot >= 1e6 ? (achiNotificationDot / 1e6).toFixed(1).replace(/\.0$/, "") + "M" : achiNotificationDot >= 1e3 ? (achiNotificationDot / 1e3).toFixed(1).replace(/\.0$/, "") + "K" : achiNotificationDot))
...
function setAchievement(o, n, e, t, i, l, s) {
    ...
    AchievementON || (AchievementON = !0), c[n] || (c[n] = { status: "locked", icon: e, name: t, description: i, type: l, progress: 0, maxProgress: s }), localStorage.setItem(`achievements_${o}`, JSON.stringify(c));
}
function addProgressAchievement(o, n, e) {
    ...
    if (t[n]) {
        ...
        if (void 0 === i) return;
        (t[n].progress += e),
            t[n].progress >= i && ((t[n].progress = i), (t[n].status = "unlocked"), console.log(`🎉 Achievement "${n}" unlocked!`)),
...
function addProgressAchievement(o, n, e) {
    let t = JSON.parse(localStorage.getItem(`achievements_${o}`)) || {};
    if (t[n]) {
        if ("unlocked" === t[n].status) return void console.log(`🏆 Achievement "${n}" is already unlocked!`);
        let i = t[n],
            l = i.maxProgress;
        if (void 0 === l) return void console.warn(`⚠️ Achievement "${n}" has no max progress defined!`);
        (i.progress += e),
            i.progress >= l && ((i.progress = l), (i.status = "unlocked"), console.log(`🎉 Achievement "${n}" unlocked!`), showAchievementNotification(i.name, i.icon, i.description)),
            localStorage.setItem(`achievements_${o}`, JSON.stringify(t)),
            document.querySelectorAll(".achievement-info progress").forEach((o) => {
                o.closest(".achievement").querySelector(".title").innerText === i.name && ((o.value = i.progress), (o.max = l));
            }),
...
function showAchievementNotification(o, n, e) {
    ...
    t || ((t = document.createElement("div")), (t.id = "achievement-container"), document.body.appendChild(t));
...
function displayAchievements(o) {
    ...
    if (((e.innerHTML = ""), 0 === Object.keys(n).length)) return console.warn(`⚠️ No achievements found for mod: ${o}`), void (e.innerHTML = "<p>No achievements yet!</p>");
...
OnDelayIn(1e3, () => {
    AchievementON || document.getElementById("tab-myachi").remove(), showAchiNotificationDot();
}),
    OnClick("tab-myachi", function () {
        (achiNotificationDot = 0), localStorage.setItem("achiNotificationDot", achiNotificationDot), showAchiNotificationDot();
    });
...
// MIX FOR PEACE CODING HERE!!!!


// Register your mod here
var RegisterMod = "Axe Boat";
var RegisterModVersion = "0.1.7.1"; // small update to the hd video no achivements fixes
var ModDeveloper = "Rizsim Studios";
...
let pictosCentered = true;

addEventListener('click', function() { // FIXES PICTO SIZE
  if(pictosCentered) {
    let pictosToMove = document.querySelectorAll("#picto20.picto");
    let pictolineBot = document.querySelector(".pictoline.bot");

    if (pictosToMove.length > 0 && pictolineBot) {
        pictosToMove.forEach(picto => {
            pictolineBot.appendChild(picto);
			
			  document.querySelector(".pictoline.top").style.cssText = `
              display: flex;
              justify-content: center;
              width: 98%;
          `;
                  
          document.querySelector(".pictoline.bot").style.cssText = `
              display: flex;
              justify-content: center;
              width: 112%;
			  margin-left: 2%;
...
function onV1Start() {
	OnDelayIn(700, () => {
      unlockAchievement(RegisterMod, "ach_0");
    });
}
...
function onV1Polo6() {
	  engimaticActive = true;
}

function offV1Polo6() {
	engimaticActive = false;
}
...
function onV1Mix() { // for goofy axe boat :)
  onBPM(120, () => {
    feelMFPActive && feelActive && unlockAchievement(RegisterMod, "ach_2");
//	kickActive && snareActive && toutiActive && engimaticActive && odoyounoActive && ouaActive && unlockAchievement(RegisterMod, "ach_3"),
//	feelActive && yeahActive && unlockAchievement(RegisterMod, "ach_5");
  });
//  onBPM(140, () => {
//  kickActive && odoyounoActive && unlockAchievement(RegisterMod, "ach_6");
// });
}
function offV1Mix() {
  offBPM();
...
function onV1Bonus1Started() { // LYRICS CODE FOR MIX FOR PEACE!!
	OnDelayIn(110, function(){
	Shake(20, 570);
	QuickMessage("YEA!", 570);
	});
	OnDelayIn(890, function(){
	QuickMessage("Baby You Can't Touch This Love!", 2250);
	});
	OnDelayIn(3385, function(){
	QuickMessage("Duh..", 1000);
	});
	OnDelayIn(4800, function(){
	QuickMessage("Baby if you can't...", 980);
	});
	OnDelayIn(6000, function(){
	QuickMessage("<b style=color:#d7a267>FEEL</b>", 900);
	});
	OnDelayIn(6900, function(){
	Shake(20, 570);
	QuickMessage("<b style=color:#7a7ff6>LOVE!</b>", 990);
	});
}

function onV1Bonus1Finished() {
	 unlockAchievement(RegisterMod, "ach_4"); // unlocks bonus 1 achievement thing
}

OnDelayIn(421, function(){
    setAchievement(RegisterMod, "ach_0", "ach_welcome-mfp.png", "Welcome To Axe Boat!", "Enter the version itself", "normal");
});

OnDelayIn(430, function(){
    setAchievement(RegisterMod, "ach_1", "ach_rizcredits.png", "You Read It", "Just read the credits for this mod", "normal");
});

OnDelayIn(500, function(){
    setAchievement(RegisterMod, "ach_2", "ach_feelmfp.png", "Two Versions?", "Place Both MFP Feel and The Love Feel in your mix", "hidden");
});

// NOTE THE OTHERS ARE COMMENTED OUT BECAUSE UNLOCKING THEM ARE BUGGED FOR NOW AND I CANT FIX THAT - Rizsim Studios

OnDelayIn(621, function(){
//	setAchievement(RegisterMod, "ach_3", "ach_kickengimatic.png", "My Favorites", "Place Kick, Snare, Touti, Engimatic, Odoyouno & Oua Into the mix!", "hidden");
});

OnDelayIn(630, function(){
    setAchievement(RegisterMod, "ach_4", "ach_axeb1.png", "Feel The Early Groove", "Play the First Bonus!", "hidden");
});

OnDelayIn(650, function(){
//    setAchievement(RegisterMod, "ach_5", "ach_feelyeah.png", "Double Melody", "Place Both The Love Feel and Yeah in your mix", "hidden");
});

OnDelayIn(670, function(){
  //  setAchievement(RegisterMod, "ach_6", "ach_kickodoyouno.png", "Robots", "Place Both Kick and Odoyouno in your mix", "hidden");
});